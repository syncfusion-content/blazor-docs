...
5. Enter the project name (for example, `TreeGridApp`).
...
3. Run the following command to create a new Blazor Web App with Auto interactivity:

{% tabs %}
{% highlight c# tabtitle="Blazor Web App" %}

dotnet new blazor -o TreeGridApp -int Auto
cd TreeGridApp
cd TreeGridApp.Client
...
2. Navigate to the project directory containing the `.csproj` file.
   - For **Auto** or **WebAssembly** render modes, navigate to the **Client** project folder.
...
1. Open the `_Imports.razor` file in the `Components` folder (for Server render mode) or `Client` project (for WebAssembly/Auto render modes).
...
The `_Imports.razor` file now contains the necessary Syncfusion namespaces globally across the project.
...
2. Create a new file named `TreeGrid.razor`.

3. Add the `@rendermode` directive at the top of the file (required for Blazor Web App):

```razor
@rendermode InteractiveAuto
```

> **Note:** For Server render mode, use `@rendermode InteractiveServer`.

4. Save the file.

The component is now ready for adding the TreeGrid markup and logic.

---

## Define Row Data

1. In the `TreeGrid.razor` file, add the following code within the `@code` block:
...
@code {
    ...
    protected override void OnInitialized()
    {
        ...
        { 
            TaskId = 12, 
            TaskName = "Testing", 
            StartDate = new DateTime(2026, 02, 27),
            Duration = 8,
            Progress = 0,
            Priority = "Medium",
            ParentId = 1 
...
The component now displays a TreeGrid with hierarchical data representing a project structure. The `OnInitialized` method populates the sample data during component initialization.
...
- **ParentId**: Reference to the parent task (null for root tasks)

The hierarchical relationship is established through the `IdMapping="TaskId"` and `ParentIdMapping="ParentId"` properties on the TreeGrid.
...
1. In the `TreeGrid.razor` file, locate the `<TreeGridColumns>` section.
...
|---|---|---|
| **AllowPaging** | Enable/disable paging | false |
| **PageSize** | Number of rows per page | 12 |
| **PageSizeMode** | Paging mode | Root (paginates root-level rows only) |

**PageSizeMode Options:**

- **Root**: Paging applies to root-level rows only (recommended for TreeGrid)
- **All**: Paging applies to all rows including children

The pager automatically appears at the bottom of the TreeGrid, displaying navigation controls and page information.

> For hierarchical data, use `PageSizeMode="PageSizeMode.Root"` to keep parent-child relationships intact across pages.
...
- Click again to reverse sort direction (**ascending → descending → no sort**)
...
1. In Visual Studio, press <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>B</kbd> to build the solution.
2. Press <kbd>F5</kbd> or <kbd>Ctrl</kbd>+<kbd>F5</kbd> to start the application.
3. A browser window opens automatically and navigates to `https://localhost:7001` (or similar).
