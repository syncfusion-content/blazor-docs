...
The Syncfusion® Blazor TreeGrid includes a feature that enhances readability within the ColumnChooser dialog by allowing long column names to wrap across multiple lines. Enable this behavior by setting the [TreeGridColumnChooserSettings.AllowTextWrap](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.TreeGrid.SfTreeGrid-1.html#Syncfusion_Blazor_TreeGrid_SfTreeGrid_1_AllowTextWrap) property to **True**.

In the below demo, the ColumnChooser displays wrapped column headers, making it easier to view and select columns with long names without horizontal scrolling.
