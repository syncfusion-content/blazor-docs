---
layout: post
title: Getting Started with AutoComplete in Blazor Web App | Syncfusion®
...
If using the `WebAssembly or Auto` render modes in the Blazor Web App, install Blazor components NuGet packages in the client project.
...
N> Blazor components are available in the [nuget.org](https://www.nuget.org/packages?q=syncfusion.blazor). Refer to the [NuGet packages](https://blazor.syncfusion.com/documentation/nuget-packages) topic for available NuGet packages list with component details.
...
If using the `WebAssembly` or `Auto` render modes in the Blazor Web App, install Blazor components NuGet packages within the client project.
...
If using the `WebAssembly` or `Auto` render modes in the Blazor Web App, install Blazor component NuGet packages in the client project.
...
Add the Blazor AutoComplete component to a Razor page located under the Pages folder (e.g., Pages/Home.razor) in either the **Server** or **Client** project. If an interactivity location as `Per page/component` in the web app, define a render mode at top of the component, as follows:
...
* Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to launch the application. This will render the Blazor AutoComplete component in the default web browser.
