---
layout: post
title: Timezone-Aware Date Highlighting in Blazor Calendar | Syncfusion®
description: Learn how to highlight the current date based on the client timezone in Blazor Calendar components using the TimeZoneOffset property.
platform: Blazor
control: Calendar
documentation: ug
---

# Timezone-Aware Date Highlighting in Blazor Calendar

By default, Blazor Calendar components use the server's local time to determine and highlight the current date ("Today"). In Blazor Server applications, the server and client may be located in different time zones, which can cause the incorrect date to be highlighted.

To address this scenario, Calendar components provide the `TimeZoneOffset` property. When this property is specified, the component calculates the current date using Coordinated Universal Time (UTC) and the provided timezone offset. This ensures that the highlighted date reflects the client's local date rather than the server date.
...
The following example demonstrates how to configure the Calendar component to highlight today's date based on a client timezone offset.
