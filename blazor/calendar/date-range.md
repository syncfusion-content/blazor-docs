...
* If the `Value` is out of the specified date range, dates outside the range are disabled in the calendar UI. When the user selects a date, the selected value is clamped to the nearest bound (less than `Min` is clamped to `Min`; greater than `Max` is clamped to `Max`).
