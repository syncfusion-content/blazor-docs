...
6. Now, manually add the custom styles in the Blazor App to render the components without any issues on the machines that contains no internet access.
    ...
    * For **Blazor Server App**, reference custom styles in
    ...
    ```
### Using Customized Styles from Syncfusion.Blazor.Themes Package and CDN (Version 23.2.4 and above) 

Starting from version 23.2.4, customized Material and Tailwind themes are available in the Syncfusion.Blazor.Themes package and can be accessed via CDN for Blazor components.
