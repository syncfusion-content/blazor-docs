...
Icon component provides support to render predefined icons or custom font icons.
...
The built-in icons can be integrated with other Blazor components without defining the `<SfIcon>` tag. To use icons, add `e-icons` class that contains the font-family and common property of the font icons. Add the icon class with the corresponding icon name from the [available icons](#icons-list) with `e-` prefix.
...
Built-in icons can be rendered directly in an HTML element by adding the `e-icons` class (font family and common properties) and the [available icon](#icons-list) class with the `e-` prefix.

The following code example explains the direct rendering of `search` icon in the span element.
...
The complete pack of Blazor icons is listed in the following table. The corresponding icon content can be referred to the content section.
