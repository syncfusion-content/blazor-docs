...
If you are using **OpenAI**, [Create an API key](https://github.com/syncfusion/smart-ai-samples/tree/master/blazor#openai) and place it at `apiKey`, leave the `endpoint` as `""`. The value for `deploymentName` is the [models](https://github.com/syncfusion/smart-ai-samples/tree/master/blazor#openai) you wish to use (e.g., `gpt-3.5-turbo`, `gpt-4`, etc.).
