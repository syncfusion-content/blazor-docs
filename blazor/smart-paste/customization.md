---
layout: post
title: Customization in Blazor Smart Paste Button Component | Syncfusion
description: Checkout and learn here all about Styles and Appearances in Syncfusion Blazor Smart Paste Button component and more.
platform: Blazor
control: Smart Paste Button
documentation: ug
---

# Types and Appearance

The Syncfusion&reg; Blazor Smart Paste Button component fully inherits all the properties, types, and styling options of the Syncfusion&reg; Blazor Button component. This means that you can leverage the existing features of the Syncfusion&reg; Blazor Button while benefiting from the enhanced functionality of the Smart Paste Button.

* [Types and Styles](https://blazor.syncfusion.com/documentation/button/types-and-styles)
* [Appearances](https://blazor.syncfusion.com/documentation/button/style-and-appearance)

## See also

* [Syncfusion&reg; Blazor Button Types and Styles](https://blazor.syncfusion.com/documentation/button/types-and-styles)
* [Getting Started with Syncfusion&reg; Blazor Smart Paste Button in Blazor Web App](https://blazor.syncfusion.com/documentation/)
* [Getting Started with Syncfusion&reg; Blazor Smart Paste Button in Blazor Server App](https://blazor.syncfusion.com/documentation/)
