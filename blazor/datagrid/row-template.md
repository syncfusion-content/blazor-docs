...
Enable the row template feature by setting the `rowTemplate` property of the Data Grid component. This property accepts an template that defines the custom layout for each row, providing access to row data through template context.
...
## Render Chart component in row templates
...
The row template feature replaces the grid’s default row rendering with a custom layout. Since this fully changes the standard structure with its own content presentation, it is not compatible with built‑in data operations or advanced layout options.

## See also

- [Column Template](https://blazor.syncfusion.com/documentation/datagrid/column-template)
- [Header Template](https://blazor.syncfusion.com/documentation/datagrid/column-headers#header-template)
- [Empty Record Template](https://blazor.syncfusion.com/documentation/datagrid/how-to/customize-empty-grid-display-message)