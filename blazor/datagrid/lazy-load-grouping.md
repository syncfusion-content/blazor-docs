...
When enabling lazy load grouping with the infinite scrolling feature, the Grid will initially render only the caption rows in a collapsed state, based on the  [InitialBlocks](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Grids.GridInfiniteScrollSettings.html#Syncfusion_Blazor_Grids_GridInfiniteScrollSettings_InitialBlocks) property of the [GridInfiniteScrollSettings](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Grids.GridInfiniteScrollSettings.html) class, at the time of grouping. The infinite scrolling feature supports parent-level caption rows. The next set of caption rows will be fetched once the scrollbar reaches the end of the scroller. To load child items, a virtual request is sent to retrieve data for the expanded caption rows. The caption row expand/collapse state will be persisted while scrolling. In addition, for great performance, a loading placeholder indicator(masked row) is shown between the time of fetching the new data and binding it to the grid for child items rendering.
