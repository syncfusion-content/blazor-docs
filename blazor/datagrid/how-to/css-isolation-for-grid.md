...
## Why use CSS isolation with Data Grid

When working with the Blazor Data Grid (`SfGrid`), styling can become complex due to its rich structure and nested elements. The Data Grid includes multiple layers such as headers, rows, cells, and templates, each rendered with its own set of classes and DOM hierarchy. Applying global `CSS` in such scenarios may unintentionally affect other components or lead to styling conflicts across the application.

`CSS` isolation helps avoid these issues by restricting styles to a specific component. This ensures better maintainability and predictable styling behavior, especially when multiple Data Grid instances are used in different parts of the application. Styles can be applied with confidence because other components are not affected.

It also allows safe customization of the Data Grid’s appearance. For example, row colors, header styles, or cell formatting can be modified without worrying about breaking styles in other UI sections. This makes `CSS` isolation an effective approach for implementing consistent and controlled UI enhancements.

## How CSS isolation works

The `SfGrid` renders multiple internal elements such as rows, headers, and cells. Since these elements are deeply nested within the component structure, applying styles through isolated CSS requires the use of the **::deep** combinator.

The **::deep** selector enables styles defined in a **.razor.css** file to penetrate the scoped boundary and target child elements generated by the Data Grid. Without using this selector, styles defined in isolated `CSS` would only apply to the top-level component and would not affect nested elements within the Data Grid.

To ensure proper application of isolated styles, wrap the `SfGrid` component inside a standard HTML element such as a `div`. Assign a custom class to this wrapper and use the same class in the isolated CSS rule. This connection makes it clear which wrapper the CSS targets.

This setup ensures that styles are applied precisely where needed while still maintaining the benefits of `CSS` isolation. It also simplifies targeting specific Data Grid elements and avoids unintended style leakage across the application.

## Applying CSS isolation to Data Grid

Below is an example showing how to define the Data Grid markup in `Index.razor` and how the wrapper class connects to the isolated CSS rule.
...
`CSS` isolation provides a structured and maintainable way to customize the appearance of the Blazor Data Grid while preventing style conflicts across the application.
...
N> More information on CSS isolation is available [here](https://learn.microsoft.com/en-us/aspnet/core/blazor/components/css-isolation#child-component-support).
