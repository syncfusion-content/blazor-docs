---
layout: post
title: Blazor Grid Use Radio Button Instead of Checkbox | Syncfusion
description: Learn how to use radio buttons instead of checkboxes in Blazor Data Grid for single-row selection using templates and the ValueChange event.
platform: Blazor
control: DataGrid
documentation: ug
---

# How to use Radio Button Instead of Checkbox in Blazor Data Grid

By default, the [Blazor Data Grid](https://www.syncfusion.com/blazor-components/blazor-datagrid) provides checkbox selection for multiple row selection. When only one row should be selectable at a time, radio buttons can be used instead of checkboxes. This is achieved by using the [Column Template](https://blazor.syncfusion.com/documentation/datagrid/column-template) feature to render an [SfRadioButton](https://blazor.syncfusion.com/documentation/radio-button/getting-started-webapp) in each row. Assign the same radio group name for all rows so only one radio button can be selected at a time, and bind each radio button to a unique value from the data source (typically the primary key field).
...
<SfGrid @ref="GridInstance" DataSource="@Orders" AllowSelection="true" TValue="Order">
        ...
        <GridColumn>
            <Template>
                @{
                    ...
                    <SfRadioButton Name="RadioBtn" Value="@PrimaryVal?.CustomerID" ValueChange="ValueChange" TChecked="string"></SfRadioButton>
                }
            </Template>
        </GridColumn>
...
{% previewsample "https://blazorplayground.syncfusion.com/embed/rXLHXbrgxmGKuyxt?appbar=false&editor=false&result=true&errorlist=false&theme=fluent2" %}
