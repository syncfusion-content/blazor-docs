...
The following example renders a customer rating column with a custom template. Inside the template, the code iterates over a fixed range and generates star elements. The `context` object refers to the current row data item, and the **IsRatingGreater** helper method determines which stars to highlight.
...
{% previewsample "https://blazorplayground.syncfusion.com/embed/rDVdDmtisvkVUGFo?appbar=false&editor=false&result=true&errorlist=false&theme=fluent2" %}