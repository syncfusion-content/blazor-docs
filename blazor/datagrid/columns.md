...
By default, a filter query for the foreignkey column can be generated based on the foreignkey value. When the URL query string is too long (containing the filter queries), exception occurs while performing filtering operations. To overcome this issue, we can prevent filter query generation for the foreignkey column. This can be achieved by setting the PreventFilterQuery argument of the OnActionBegin event to true.
