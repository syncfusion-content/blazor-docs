...
To customize the Checkbox/Excel list element **`FilterItemTemplate`** can also be used .But it is used only for display purpose and the default searching will perform based on the column's field value only. So to perform search operation, we have provided support to **`CheckBoxListData`** where custom data list can be passed to filter dialog. To pass custom datasource for the checkbox list data in a Grid column filter you can use [CheckboxListData](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Grids.ActionEventArgs-1.html#Syncfusion_Blazor_Grids_ActionEventArgs_1_CheckboxListData) inside [OnActionBegin](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Grids.GridEvents-1.html#Syncfusion_Blazor_Grids_GridEvents_1_OnActionBegin) event of the Grid.
...
{% previewsample "https://blazorplayground.syncfusion.com/embed/BXVqtvZfUSAqiIkp?appbar=false&editor=false&result=true&errorlist=false&theme=bootstrap5" %}
