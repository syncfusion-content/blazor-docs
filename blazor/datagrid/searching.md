...
To provide a search box in the UI, add the Search item to the toolbar using the [Toolbar](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Grids.SfGrid-1.html#Syncfusion_Blazor_Grids_SfGrid_1_Toolbar) property. Searching can also be performed programmatically using [SearchAsync](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Grids.SfGrid-1.html#Syncfusion_Blazor_Grids_SfGrid_1_SearchAsync_System_String_). The [AllowSearching](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Grids.GridColumn.html#Syncfusion_Blazor_Grids_GridColumn_AllowSearching) property is column-level and is used to include or exclude specific columns from search; it is not required to enable global search.
...
The following example applies an initial search using GridSearchSettings with:
1. Fields: CustomerID targets the CustomerID column.
2. Operator: contains matches records that contain the key.
3. Key: Ha is used as initial search text.
4. IgnoreCase: true performs case-insensitive matching.
5. IgnoreAccent: true ignores diacritics during matching.
...
The following search operators are supported in the Syncfusion Blazor DataGrid:
...
The following example shows how to change the operator via a [DropDownList](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.DropDowns.SfDropDownList-2.html#Syncfusion_Blazor_DropDowns_SfDropDownList_2__ctor) and bind it to the `SearchSettings.Operator` property.
...
In the following example, searching is disabled for the `Order ID` column:
...
The following example shows how to configure `IgnoreAccent` in `GridSearchSettings`:
{% tabs %}
{% highlight razor tabtitle="Index.razor" %}
...
The following example shows how to clear searched records using an external button.
