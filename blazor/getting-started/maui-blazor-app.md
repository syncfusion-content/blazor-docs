...
This section explains how to create and run the first .NET Multi-platform Blazor App UI (.NET MAUI Blazor) app with Syncfusion&reg; Blazor components.
...
Now add Syncfusion&reg; Blazor component in any razor file. Here, the Calendar component is added in `~/Pages/index.razor` page under the `~/Pages` folder.
...
![MAUI Blazor App with Syncfusion&reg; Blazor Components](images/maui/maui-blazor-calendar.png)
...
In iOS code is statically compiled ahead of time, so, configure Syncfusion&reg; Blazor assemblies in `MtouchExtraArgs` tag for the iOS Release configuration in the project when deploy on a real device.
