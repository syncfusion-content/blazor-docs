---
layout: post
title: Get tab item through Id | Syncfusion
description: Checkout and learn here all about how to get tab item through Id in Syncfusion Blazor Tabs component and much more details.
platform: Blazor
control: Tabs
documentation: ug
---

#  Get tab item through Id
Tabs provides support for getting tab items using unique **Id**. The tab items can be called by passing the specified Id through `GetTabItemById` method.

In the following demo, specified tab item's header text is called through its unique id when the **Get Tab items** button is clicked.
...
<button @onclick='()=>TabItemById("1")'>Get Tab items</button>
<br /><br />
...
@code{
    ...
    public void TabItemById(string id)
    {
        ...
        if(items != null)
        {
            Logs = "Header of the tab item for the given Id: "+ items.Header.Text;
        }
 ...
 ![Getting TabItem through Id in Blazor Tabs](../images/blazor-tabs-id.png)
