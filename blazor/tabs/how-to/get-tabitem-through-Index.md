---
layout: post
title: Get tab item through Index | Syncfusion
description: Checkout and learn here all about how to get tab item through index in Syncfusion Blazor Tabs component and much more details.
platform: Blazor
control: Tabs
documentation: ug
---
...
Tabs provides support for getting tab items using its unique **Index**. The tab items can be called by passing the specified Id through `GetTabItemByIndex` method.

In the following demo, specified tab item's header text is called through its unique index when the **Get Tab items** button is clicked.
...
<button @onclick='()=>TabItembyIndex(1)'>Get Tab items</button>
<br /><br />
...
@code {
    ...
    public void TabItembyIndex(int index)
    {
        ...
        if (items != null)
        {
            Logs = "Header of the tab item for the given Index : " + items.Header.Text;
        }
