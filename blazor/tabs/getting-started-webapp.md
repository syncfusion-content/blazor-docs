---
layout: post
title: Getting Started with Tabs Component in Blazor Web App | Syncfusion®
description: Check out the documentation for getting started with Blazor Tabs Components in Blazor Web App and much more details.
platform: Blazor
control: Tabs
documentation: ug
---
...
Add the Blazor Tabs component in the **~/Components/Pages/*.razor** file. If the interactivity location is set to `Per page/component` in the Web App, define a render mode at the top of the `~/Pages/*.razor` file. (For example, `InteractiveServer`, `InteractiveWebAssembly` or `InteractiveAuto`).
...
* Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to launch the application. This will render the Blazor Tabs component in the default web browser.
