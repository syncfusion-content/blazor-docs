...
Add the Blazor Message component in the **~/Components/Pages/*.razor** file. If the interactivity location is set to `Per page/component` in the Web App, define a render mode at the top of the `~/Pages/*.razor` file. (For example, `InteractiveServer`, `InteractiveWebAssembly` or `InteractiveAuto`).
...
{% previewsample "https://blazorplayground.syncfusion.com/embed/rZrnZngDVzFbDEkL?appbar=false&editor=false&result=true&errorlist=false&theme=fluent2" backgroundimage "[Blazor Message Component](./images/message-default.webp)" %}
