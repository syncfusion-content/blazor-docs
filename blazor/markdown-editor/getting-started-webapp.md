...
Add the Syncfusion Blazor Markdown Editor component in the **~/Components/Pages/*.razor** file. If the interactivity location is set to `Per page/component` in the Web App, define a render mode at the top of the **~/Pages/Index.razor** file. file. (For example, `InteractiveServer`, `InteractiveWebAssembly` or `InteractiveAuto`).
...
{% previewsample "https://blazorplayground.syncfusion.com/embed/LNrnjSBwgamUlaQo?appbar=false&editor=false&result=true&errorlist=false&theme=bootstrap5" backgroundimage "[Blazor Markdown Editor Component](./images/blazor-markdownteditor.png)" %}
