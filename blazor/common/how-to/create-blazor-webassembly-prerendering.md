...
7. Add Syncfusion&reg; Blazor service in the `~/Server/Program.cs` file.
...
The Syncfusion&reg; Blazor service has registered the HttpClient service itself by default. When you run the `WebAssemblyPrerendered` mode application, it tries to get the WebAPI with its absolute URI or BaseAddress.
