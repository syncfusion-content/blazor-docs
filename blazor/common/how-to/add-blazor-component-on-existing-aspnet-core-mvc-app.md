...
This section explains how to add Syncfusion&reg; Blazor component on an existing ASP.NET Core MVC application.
...
4. Register Blazor server service and Syncfusion&reg; Blazor service in the `~/Program.cs` file.
...
7. Add Blazor script references at the end of `<body>` tag and Syncfusion&reg; theme and script references inside the `<head>` tag on `~/Views/Shared/_Layout.cshtml` file.
...
9. Add the Syncfusion&reg; Blazor component in the created razor file.
...
11. Run the application by pressing `F5` key. Now, the Syncfusion&reg; Blazor Grid component will be rendered in the ASP.NET Core MVC application.
