...
* Create deltas: Use [GetDiagramUpdates]() to produce a compact set of incremental updates (JSON-formatted deltas) representing just the changes, not the entire diagram.
...
* Apply remotely: Each remote user listens for ReceiveData and applies the incoming deltas with [SetDiagramUpdatesAsync](), keeping their view synchronized without reloading the full diagram.
* Group interactions: Enable [EnableGroupActions]() in [DiagramHistoryManager]() to treat multi-step edits (like drag/resize sequences or batch changes) as one operation. When enabled, `HistoryChanged` raises [StartGroupAction]() and [EndGroupAction](). Send updates after `EndGroupAction` to reduce network traffic (one consolidated update) and apply changes atomically for all collaborators.
