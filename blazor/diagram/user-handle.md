...
### How to set the displacement of the user handle
...
### How to set the Margin of the user handle

The [Margin](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Diagram.UserHandle.html#Syncfusion_Blazor_Diagram_UserHandle_Margin) property of user handles is applicable only for nodes and allows you to set the spacing around the user handle within the node's boundary. The Margin property enables you to define the precise positioning of the user handle by adding padding space on all sides (Top, Bottom, Left, Right) of the handle. This property is particularly useful when you want to create visual separation between the user handle and the node's edges for better accessibility and visual clarity.

The following code explains how to customize margin of the User Handle for nodes.
...
@code
{
    ...
    protected override void OnInitialized()
    {
        ...
        UserHandle cloneHandle = new UserHandle()
        {
            ...
            Side = Direction.Bottom,
            //Set margin for the user handle with spacing on all sides.
