...
description: Checkout and learn here all about Lane in Syncfusion Blazor Diagram component, its elements and much more.
platform: Blazor
...
* You can create the `Lane` and add into the `Lanes` collection of the Swimlane.
...
* The size of lane header can be controlled by using the `Width` and `Height` properties of header.
* The appearance of lane header can be set by using the `Style` properties.
 ...
 * You can able to resize the individual lane header. While resizing lane, it will maintain 20px distances from the Lane children.
