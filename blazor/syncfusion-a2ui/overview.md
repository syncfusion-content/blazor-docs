...
- **`Syncfusion.Blazor.A2UI`** — the Blazor renderer. Adds authentic Syncfusion EJ2 Blazor widgets on top of the Core engine, plus the 18 A2UI primitives (`Text`, `Row`, `Column`, `Card`, `Tabs`, `Modal`, `Button`, etc.) as HTML-fallback implementations.
...
<!-- Both packages are at 0.1.0-beta.0 during preview. Replace with the final NuGet package ids when published. -->
...
- Syncfusion EJ2 Blazor components installed for the widget families the agent is expected to render (`Syncfusion.Blazor.Grid`, `Syncfusion.Blazor.Charts`, `Syncfusion.Blazor.Schedule`, etc.). The set of component packages you need is determined by which A2UI adapters you plan to use — see [Supported Components](./supported-components).
...
- Ships a catalog of [more than 60 Syncfusion EJ2 Blazor adapters](./supported-components) — every one is a real Syncfusion widget — plus the 18 A2UI primitives as HTML fallbacks.
...
- **`CatalogRegistry`**: The container that holds one or more `Catalog` instances keyed by id. The same registry backs both `AddA2UI()` (basic only) and `AddA2UIWithSyncfusionComponents()` (combined). `CatalogRegistry.Register` throws `A2uiCatalogError` on duplicate ids.
...
- **`<SyncfusionA2UIProvider>`**: A Blazor component (`Syncfusion.Blazor.A2UI.NodeView.SyncfusionA2UIProvider`) that takes a `SurfaceModel` and renders it as interactive Syncfusion EJ2 components.
...
Because every adapter renders a real Syncfusion EJ2 Blazor component, the generated surfaces inherit the accessibility capabilities provided by the underlying widgets, including ARIA roles, keyboard navigation, and high-contrast theme support.

## What you get in the package

- **18 A2UI primitives** rendered as HTML fallbacks under the `BlazorSyncfusionCatalog` aggregator: `Text`, `Image`, `Icon`, `Video`, `AudioPlayer`, `Card`, `Tabs`, `Modal`, `Row`, `Column`, `List`, `Divider`, `Button`, `TextField`, `CheckBox`, `ChoicePicker`, `Slider`, `DateTimeInput` — these come from `Syncfusion.Blazor.A2UI.Catalog.BasicComponentFactory` and ship in the base Blazor package even when no Syncfusion widgets are registered.
- **`Syncfusion.Blazor.A2UI.SyncfusionComponents.SyncfusionComponentFactory`**: more than 60 Syncfusion EJ2 Blazor adapters — `DataGrid`, `TreeGrid`, `Chart`, `3DChart`, `HeatMap`, `Diagram`, `Maps`, `Scheduler`, `GanttChart`, the calendar/date family, the button family, dropdowns, inputs, editors, file/barcode generators, layout/navigation, notifications, and more. The exact list lives on [Supported Components](./supported-components).
...
- **DI helpers**: `AddA2UI()` for the basic-catalog only, `AddA2UIWithSyncfusionComponents()` for the combined basic + Syncfusion catalog.
- **Type declarations and A2uiJson serializers** (`Syncfusion.A2UI.Core.Serialization`) for every wire POCO, so the host app and any agent SDK can share one source of truth for the pipeline.
- **25 built-in functions** (`add`, `subtract`, `equals`, `contains`, `formatString`, `required`, …) under `Syncfusion.A2UI.Core.BasicCatalog.BasicFunctions` that compose inside the `${...}` data-binding expression language.
- **`Syncfusion.A2UI.Core.Markdown` integration** through `Syncfusion.Blazor.A2UI.Markdown.MarkdownService`, so `Text` components render marked-up agent output as HTML via Markdig.
...
| Blazor adapter | `Syncfusion.Blazor.A2UI` | DI extensions (`AddA2UI`), `BlazorSyncfusionCatalog`, `BlazorComponentAdapter`, `EventDispatchHelper`, `<SyncfusionA2UIProvider>`, Markdown service. |
