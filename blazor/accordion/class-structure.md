...
| CSS Class | Description |
|-----------|-------------|
| `.e-toggle-icon` | Applied to the container that holds the expand and collapse indicator icon. Positioned absolutely on the right side (or left in RTL mode) of the header with specific height and width constraints. |
...
| CSS Class | Description |
|-----------|-------------|
| `.e-acrdn-item.e-select` | Applied to the Accordion items that are interactive and can be expanded or collapsed. Items with this class show pointer cursor on hover and have hover/focus text decoration effects. |
...
| `.e-acrdn-item.e-expand-state` | Applied to Accordion items in their expanded state. Used in combination with `.e-selected` to identify items that are currently open and displaying their content panel. |
...
| `.e-acrdn-item.e-select.e-selected.e-expand-state` | Applied to selectable Accordion items that are currently selected and expanded. Represents the complete expanded item state with all relevant modifiers. |
