...
Apart from the default arrangement of the Scheduler events, the user can cusom sorting the events based on the custom fields for appointments. The following code example shows how to sort the appointments based on the custom field **RankId** assigned to the `SortBy` property.
