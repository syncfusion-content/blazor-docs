---
layout: post
title: Set different work hours for each days of the each resource in blazor schedule | Syncfusion
...
# Set different work hours for each days of the each resource in Blazor Scheduler Component

In Blazor, you can set the required work hours on Scheduler, thus visually highlighting the cells underlying the given work hour range. In the following code example, you can set the different work hours for each appointment based on the day of the week by using the `SetWorkHoursAsync` method. Before setting up the custom work hours we need to reset the default work hours by using the `ResetWorkHoursAsync` method.

```cshtml
@using Syncfusion.Blazor.Schedule

<SfSchedule @ref="@ScheduleRef" TValue=DTO_TimeSlotTemplate CurrentView="View.Week" Timezone="@TimeZone" ShowWeekend="false" WorkDays="@WorkingDays">
    ...
    <ScheduleEvents TValue="DTO_TimeSlotTemplate" Created="OnCreated" ActionCompleted="OnActionCompleted" DataBound="OnDataBound"></ScheduleEvents>
</SfSchedule>

@code {
    SfSchedule<DTO_TimeSlotTemplate>? ScheduleRef;
    ...
    List<DTO_TimeSlotTemplate> TimeSlots = new List<DTO_TimeSlotTemplate>
    {
        new DTO_TimeSlotTemplate { Id = 1, Subject = "Daily Medical check up", StartTime = new DateTime(2022, 7, 17, 14, 30, 0) , EndTime = new DateTime(2022, 7, 17, 15, 30, 0), RecurrenceRule = "FREQ=WEEKLY;BYDAY=SU,MO,TU,TH,FR,SA;INTERVAL=1;", DoctorID = 1 },
        new DTO_TimeSlotTemplate { Id = 2, Subject = "Weekly Medical check up", StartTime = new DateTime(2022, 7, 17, 15, 30, 0) , EndTime = new DateTime(2022, 7, 17, 15, 45, 0), RecurrenceRule = "FREQ=WEEKLY;BYDAY=SU;INTERVAL=1;", DoctorID = 2 }
    };
    private async Task OnCreated() {
        await SetWorkHours(); // Setting work hours after the intial rendering
    }
    private void OnActionCompleted(ActionEventArgs<DTO_TimeSlotTemplate> args)
    {
    ...
    private async Task OnDataBound(DataBoundEventArgs<DTO_TimeSlotTemplate> args)
    {
        if (IsLayoutChanged)
        {           
    ...
    public class DTO_TimeSlotTemplate
    {
