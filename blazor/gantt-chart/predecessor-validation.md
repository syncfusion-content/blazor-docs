...
In the below example, the connector line which is connected from task id 2 to task id 3 is rendered at load time. Hence validation happens. Whereas, the connector line from task id 6 and task id 7 is drawn dynamically and validation is restricted here by disabling the `EnableAutoLinkValidation` property.
...
![Disabling predecessor validation on predecessor drawing](images/auto-link-validation.png)
