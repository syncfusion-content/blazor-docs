...
N>  When specifying a predecessor by using only the task `Id` without mentioning a dependency type, the default dependency type of **Finish-to-Start (FS)** is automatically applied.
...
To specify multiple predecessors for a task, they can be defined as a comma-separated string. Each predecessor is identified using the task `Id` and its corresponding dependency type (e.g., "2FS", "6SS"). This approach facilitates detailed and synchronized scheduling, accommodating intricate project dependencies.
...
You can define the predecessor types in any order. The default order will be FS, SS, FF, and SF. The types can be configured, as per the requirement by using an enum type namely `DependencyTypes`, and the type mentioned in the first index of the `DependencyTypes` property will be the default type.
