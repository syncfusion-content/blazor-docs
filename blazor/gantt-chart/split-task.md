...
The segment collection also supported for ExpandoObject, DynamicObject, and adaptors way to initialize the segments collection in Gantt Chart. Refer the demo [link](https://blazor.syncfusion.com/demos/gantt-chart/split-tasks?theme=fluent).

### Split Task

  `Dialog`: Segments tab is rendered in add/edit dialog, when the GanttSegmentFields component is mapped. Using this tab, we can split the task based on the original start and end date of a particular task.

  `Context menu`: When the GanttSegmentFields component is mapped and the EnableContextMenu property is enabled, Split Task item will be included in the context menu.

  `SplitTaskAsync`: When the GanttSegmentFields component is mapped and use the method split the particular taskbar based on given task id and dates. Refer the [SplitTaskAsync](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Gantt.SfGantt-1.html#Syncfusion_Blazor_Gantt_SfGantt_1_SplitTaskAsync) method.
...
### Merge Task
  ...
  `MergeTaskAsync`: When the GanttSegmentFields component is mapped and use the method to merge the particular segments based on given task id and index values, the index values starts with 0. refer the [MergeTaskAsync](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor.Gantt.SfGantt-1.html#Syncfusion_Blazor_Gantt_SfGantt_1_MergeTaskAsync) method.
